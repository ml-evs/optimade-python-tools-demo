...
app = FastAPI(
    title="OPTIMADE API",
    description=(
        f"""The [Open Databases Integration for Materials Design (OPTIMADE) consortium](https://www.optimade.org/) aims
to make materials databases interoperational by developing a common REST API.

This specification is generated using
[`optimade-python-tools`](https://github.com/Materials-Consortia/optimade-python-tools/tree/v{__version__}) v{__version__}.
"""
    ),
...
if not CONFIG.use_real_mongo:
    ...
    with open("./structures.json", "r") as f:
        data["structures"] = bson.json_util.loads(f.read())
    with open("./references.json", "r") as f:
        data["references"] = bson.json_util.loads(f.read())
    ...
    def load_entries(endpoint_name: str, endpoint_collection: MongoCollection):
        ...
        if endpoint_name != "links":
            endpoint_collection.collection.insert_many(data.get(endpoint_name, []))
        if endpoint_name == "links":
